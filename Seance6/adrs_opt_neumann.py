# -*- coding: utf-8 -*-
"""
Created on Mon Oct 13 16:50:04 2025
...
# -*- coding: utf-8 -*-
"""
Analyse du comportement de l'optimisation ADRS avec condition de Neumann à la sortie
Auteur : SCD UM
Date : Oct 2025
"""
...
def ADRS(NX, xcontrol, Cible, adapt_mesh=False):
    ...
    # Terme source (en fonction des contrôles)
    for j in range(1, NX - 1):
        for ic in range(len(xcontrol)):
            F[j] += xcontrol[ic] * np.exp(-100 * (x[j] - L / (ic + 1))**2)

    # Ajustement du pas de temps (condition CFL)
    ...
    while n < NT and res > eps * res0:
        ...
        # Calcul du schéma aux points internes
        for j in range(1, NX - 1):
            xnu = K + 0.5 * dx * abs(V)
            Tx = (T[j + 1] - T[j - 1]) / (2 * dx)
        ...
        # Bord gauche (Dirichlet : T(0)=0 par défaut)
        T[0] = 0.0

        # Bord droit (condition de Neumann : ∂T/∂x = 0)
        # → dérivée nulle => T[N-1] ≈ T[N-2]
        # ou plus stable : extrapolation linéaire
        T[-1] = 2 * T[-2] - T[-3]
...
plt.title("Comparaison des solutions optimales (condition de Neumann en sortie)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
