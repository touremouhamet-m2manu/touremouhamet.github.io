...
erreur_L2 = np.sqrt(np.sum((T - u_ex(x))**2))
print(f"Erreur L2 entre T et u_ex : {erreur_L2}")

# Tracé de la comparaison
plt.figure()
plt.plot(x, T, label="Solution numérique $u_h(s_h)$")
plt.plot(x, u_ex(x), '--', label="Solution exacte $u_{ex}(s_h)$")
plt.xlabel('$s$')
plt.ylabel('$u$')
plt.title('Comparaison entre $u_h(s_h)$ et $u_{ex}(s_h)$')
plt.legend()
plt.show()
